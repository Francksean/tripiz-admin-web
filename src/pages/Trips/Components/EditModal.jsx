...
import {X, Save, Bus, User, MapPin, Calendar, Clock, AlertCircle, Map} from 'lucide-react';
...
// IMPORTANT: on récupère bien buses, drivers, itineraries envoyés par le parent
export const EditTripModal = ({ isOpen, onClose, onSave, trip, buses = [], drivers = [], itineraries = [] }) => {
    ...
    useEffect(() => {
        if (isOpen && trip) {
            setFormData({
                // On gère les deux conventions possibles (snake_case direct ou *Id camelCase)
                bus_id:             trip.bus_id ?? trip.busId ?? '',
                driver_id:          trip.driver_id ?? trip.driverId ?? '',
                itinerary_id:       trip.itinerary_id ?? trip.itineraryId ?? '',
                trip_date:          trip.trip_date || '',
                schedule_departure: (trip.schedule_departure || '').substring(0, 5), // format HH:MM pour l'input time
                trip_status:        trip.trip_status || trip.status || 'PROGRAMME',
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const payload = {
                ...
                schedule_departure: formData.schedule_departure.length === 5
                    ? `${formData.schedule_departure}:00`
                    : formData.schedule_departure,
            ...
            await onSave({ trip_id: tripId, ...payload });
            onClose();
        } catch (err) {
            console.error(err);
        } finally {
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-sm font-semibold text-gray-900">
                                Modifier le trajet <span className="text-gray-400 font-normal">#{trip.trip_id || trip.id}</span>
                            </h2>
                ...
                <form onSubmit={handleSubmit} className="px-5 py-4 space-y-4">
                    ...
                    <div className="bg-gray-50 rounded-xl p-4 space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <Field label="Bus *" icon={Bus} error={errors.bus_id}>
                                <select value={formData.bus_id} onChange={(e) => handleChange('bus_id', e.target.value)} className={getInputCls(!!errors.bus_id)}>
                                    <option value="">Sélectionner…</option>
                                    {buses.map(b => <option key={b.id} value={b.id}>{b.label}</option>)}
                                </select>
                            </Field>
                            <Field label="Chauffeur *" icon={User} error={errors.driver_id}>
                                <select value={formData.driver_id} onChange={(e) => handleChange('driver_id', e.target.value)} className={getInputCls(!!errors.driver_id)}>
                                    <option value="">Sélectionner…</option>
                    ...
                    <div className="bg-gray-50 rounded-xl p-4 space-y-3">
                        ...
                        <Field label="Itinéraire *" icon={MapPin} error={errors.itinerary_id}>
                            <select value={formData.itinerary_id}
                    ...
                    <div className="bg-gray-50 rounded-xl p-4 space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <Field label="Départ prévu *" icon={Clock} error={errors.schedule_departure}>
                                <input type="time" value={formData.schedule_departure} onChange={(e) => handleChange('schedule_departure', e.target.value)} className={getInputCls(!!errors.schedule_departure)} />
                            </Field>
                        </div>
                    </div>
